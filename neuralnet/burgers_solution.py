...
class BurgersSolution:
    def __init__(self, sample_name, training_data_dir=DEFAULT_TRAINING_DATA_DIR):
        ...
        self.domain_length = (self.config[NUM_DOMAIN_POINTS_KEY] - 1) * self.config[SPATIAL_STEP_SIZE_KEY]
    ...
    def calculateArtificialViscosity(self, u):
        ...
        # Simple gradient-based artificial viscosity
    ...
    """
    for t, u in enumerate(self.solution_history):
        av_array = self.calculateArtificialViscosity(u)
        for x_idx, av_value in enumerate(av_array):
            # ux criterion from Lax-Wendroff / FTCS
            if x_idx == 0:
                ux = (u[1] - u[self.num_domain_points - 2]) / (2.0 * self.spatial_step_size)
            else:
                ux = (u[x_idx + 1] - u[x_idx - 1]) / (2.0 * self.spatial_step_size)
            if ux < 0:
                yield t, x_idx, av_value
