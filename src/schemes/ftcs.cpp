...
void FTCS::calculateNextU(const std::vector<double>& u,
  ...
  for (int i = 1; i < num_domain_points - 1; ++i) {
    u_next[i] = u[i] -
                u[i] * time_step_size / (2.0 * spatial_step_size) *
                    (u[i + 1] - u[i - 1]) +
                (kinematic_viscosity +
    ...
    double cq = (1.0 / max_ux) *
                (1.0 / (2.0 * time_step_size) -
                 kinematic_viscosity / (spatial_step_size * spatial_step_size));

    // clamping cq so that it's not negative
    cq = std::max(0.0, cq);
  }

  // wrap around
  // for some reason adding this artificial viscosity drastically slows the sim
  // down. u_next[0] = u[0]
  //     - u[0] * time_step_size / spatial_step_size * (u[0] -
  //     u[num_domain_points - 2])
  //     + (kinematic_viscosity + calculateArtificialViscosity(u, cq,
  //     spatial_step_size, 0, num_domain_points)) * time_step_size /
  //     (spatial_step_size * spatial_step_size)
  //     * (u[1] - 2 * u[0] + u[num_domain_points - 2]);
  u_next[0] = u[0] -
              u[0] * time_step_size / (2.0 * spatial_step_size) *
                  (u[1] - u[num_domain_points - 2]) +
              (kinematic_viscosity +
               calculateArtificialViscosity(u, cq, spatial_step_size, 0,
                                            num_domain_points)) *
                  time_step_size / (spatial_step_size * spatial_step_size) *
                  (u[1] - 2 * u[0] + u[num_domain_points - 2]);
...
double FTCS::calculateArtificialViscosity(const std::vector<double>& u,
                                          double cq, double spatial_step_size,
                                          int i, int num_domain_points) const {
...
std::string FTCS::getName() const { return "FTCS"; }
