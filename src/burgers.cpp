...
BurgersSolver1d::BurgersSolver1d(std::unique_ptr<BurgerScheme> scheme,
...
BurgersSolver1d::BurgersSolver1d(
    std::unique_ptr<BurgerScheme> scheme, const SolverConfig& config,
...
void BurgersSolver1d::setInitialConditions(
    const std::function<double(double)>& initial_conditions) {
...
double BurgersSolver1d::approximate_max_u() const {
  double max_u = 0.0;
  // this is a good approximation since the actual step size will be
  // (time_step_size * max_u) / ALPHA
  double approximate_step_size = time_step_size / ALPHA;
  int approximate_number_of_domain_points =
      std::floor(domain_length / approximate_step_size);
...
void BurgersSolver1d::solve(double cq) {
  ...
  for (int time_step = 0; time_step < time_steps; ++time_step) {
    scheme->calculateNextU(u, u_next, cq, num_domain_points, time_step_size,
                           spatial_step_size, kinematic_viscosity);
...
void BurgersSolver1d::saveBinarySolution(const std::string& base_folder,
  ...
  for (size_t t = 0; t < solution_history.size(); ++t) {
    ...
    file.write(reinterpret_cast<const char*>(u_t.data()),
...
void BurgersSolver1d::appendMetadata(nlohmann::json& metadata) const {
  metadata["solver"] = {{"time_steps", solution_history.size()},
...
bool BurgersSolver1d::wasNanDetected() const { return nan_detected; }
...
std::string BurgersSolver1d::getSchemeName() const { return scheme->getName(); }